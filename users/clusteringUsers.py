...
def clustering_user(files):
    ...
    def diff(dist1, dist2):
        ...
        while itr1 < len1 and itr2 < len2:
            if vec1[itr1][0] < vec2[itr2][0]:
                result += vec1[itr1][1] * vec1[itr1][1]
                itr1 += 1
            elif vec1[itr1][0] > vec2[itr2][0]:
                result += vec2[itr2][1] * vec2[itr2][1]
                itr2 += 1
            else:
                result += (vec1[itr1][1] - vec2[itr2][1]) ** 2
                itr1 += 1
                itr2 += 1
        while itr1 < len1:
            result += vec1[itr1][1] * vec1[itr1][1]
    ...
    def average(dist, k):
        ...
        for itr in range(len(result)):
            result[itr][1] = float(result[itr][1]) / dist
    ...
    all_users.reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1])) \
    ...
    kPoints = data.takeSample(True, K, 1)
    temp_dist = 1.0
    ...
    while temp_dist > CONVERGE:
        ...
        pointStats = closest.reduceByKey(
            lambda (p1, counter1), (p2, counter2): (union(p1, p2), counter1 + counter2))
        newPoints = pointStats.map(
            lambda (cid, (ctr, counter)): (cid, average(ctr, counter))).collect()
